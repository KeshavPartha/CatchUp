...
export function EpisodePlayer({ episode, showName }: EpisodePlayerProps) {
  ...
  const { progress, loading, isAuthenticated, updatePosition, flush } = useWatchProgress(episode);
  ...
  return (
    <section className="overflow-hidden rounded-lg border border-netflix-gray bg-black shadow-2xl">
      <div className="relative flex aspect-video items-center justify-center overflow-hidden bg-gradient-to-br from-netflix-darkGray via-black to-netflix-red/30">
        <div className="absolute inset-0 opacity-40" style={{ backgroundImage: `url(${episode.still_path})`, backgroundSize: 'cover', backgroundPosition: 'center' }} />
        <div className="relative z-10 text-center">
          <p className="mb-2 text-sm uppercase tracking-[0.3em] text-netflix-lightGray">CatchUp demo playback</p>
          <h2 className="text-2xl font-bold md:text-4xl">{showName}</h2>
          <p className="mt-2 text-netflix-lightGray">{episode.name}</p>
        </div>
      </div>
      <div className="space-y-4 p-4 md:p-6">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-2">
            ...
            <button onClick={togglePlayback} aria-label={isPlaying ? 'Pause episode' : 'Play episode'} className="rounded-full bg-white p-3 text-black hover:bg-white/80">
